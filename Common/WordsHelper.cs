...
namespace Common
{
    public class WordsHelper
    {
        ...
        public static int why_pinyin = 1;//控制 取得拼音的方式 //0 全拼，  1 首字母，  2 前面声母，最后一个汉字全拼。
        public static int last_hz_no = 0;//得到字符串中（中间可能有字母和 数字 ），最后的汉字序号，它要变成全拼，开始的都为声母
        ...
        /// <param name="why">0 全拼，  1 首字母，  2 前面声母，最后一个汉字全拼。</param>
        /// <param name="separator">连接拼音之间的分隔符。</param>
        /// <param name="initialCap">指定是否将首字母大写。</param>

        /// <returns>包含中文字符串的拼音的字符串。</returns>
        public static string hz_pinyin(string chs, string separator, bool initialCap)
        {
            if (string.IsNullOrEmpty(chs))// && string.IsNullOrEmpty(separator))
                return String.Empty;

            if (why_pinyin == 2)//控制 取得拼音的方式 //0 全拼，  1 首字母，  2 前面声母，最后一个汉字全拼。
            {// 2 前面声母，最后一个汉字全拼, 才去调用取得最后汉字的序号

                last_hz_no = index_hz_pinyin(chs, "=", true);
            }
            ...
            for (int j = 0; j < noWChar.Length; j++)
            {
                ...
                {
                    array = System.Text.Encoding.Default.GetBytes(noWChar[j].ToString(CultureInfo.InvariantCulture));
                    string s = noWChar[j].ToString(CultureInfo.InvariantCulture);
                    if (array.Length == 1)
                    {
                        b = true;
                        pySB.Append(s);
                    ...
                    else
                    {
                        if (s == "？")
                        {
                            pySB.Append(String.IsNullOrEmpty(pySB.ToString()) || b ? s : separator + s);
                            continue;
                        }
                        i1 = (short)(array[0]);
                        i2 = (short)(array[1]);
                                ...
                                if (chrAsc >= pyValue[aboutPos])
                                {
                                    ...
                                    for (int i = aboutPos + 32; i >= aboutPos; i--)
                                    {
                                        if (pyValue[i] <= chrAsc)
                                        {
                                            ...
                                            s = case_pinyin(s, j);
                                            //pySB.Append(pyName[i].Substring(0, 1).ToUpper() + pyName[i].Substring(1));
                        ...
                        else
                        {
                            ...
                            {
                                //pySB.Append(otherPinYin[pos].Substring(0, 1).ToUpper() + otherPinYin[pos].Substring(1));
                                s = otherPinYin[pos];
                                if (initialCap)
                                    s = ti.ToTitleCase(s);
        ...
        /// <returns>包含中文字符串的拼音的字符串。</returns>
        public static int index_hz_pinyin(string chs, string separator, bool initialCap)
        {
            if (string.IsNullOrEmpty(chs))// && string.IsNullOrEmpty(separator))
                return 0;
            //  chs = Phrase.Cast<DictionaryEntry>().Aggregate(chs, (current, de) => current.Replace(de.Key.ToString(), String.Format(" {0} ", de.Value.ToString().Replace(" ", separator))));
        ...
        #region //0 全拼，  1 首字母，  2 前面声母，最后一个汉字全拼。
        private static string case_pinyin(string str, int no)
        {
            string one_hz_str = str; //str.ToUpper(); 如果需要全大写，这里设置
            string tmpStr = str; // str.ToUpper();如果需要全大写，这里设置

            //0 全拼
            if (why_pinyin == 0) return one_hz_str;// //0 全拼

            //1 首字母
            if (why_pinyin == 1) return one_hz_str.Substring(0, 1);//1 首字母

            // 2 前面声母，最后一个汉字全拼。
            if (why_pinyin == 2 && no == last_hz_no) return one_hz_str;// 2 前面声母，最后一个汉字全拼。 

            // 2 之前的都取声母。

            one_hz_str = tmpStr.Substring(0, 1);

            if (one_hz_str.ToUpper() == "A")   //A 开头都是声母 a ai an ang ao
                one_hz_str = tmpStr;
            else if (one_hz_str.ToUpper() == "O")  //O 开头都是声母  o ou
                one_hz_str = tmpStr;
            else if (one_hz_str.ToUpper() == "E")  //E 开头都是声母 e en er
                one_hz_str = tmpStr;
            else if (one_hz_str.ToUpper() == "C")  //C 开头声母C,CH
            {
                if (tmpStr.Substring(0, 2).ToUpper() == "CH")
                    one_hz_str = tmpStr.Substring(0, 2);
            }
            else if (one_hz_str.ToUpper() == "S")  //S 开头声母 S,SH
            {
                if (tmpStr.Substring(0, 2).ToUpper() == "SH")
                    one_hz_str = tmpStr.Substring(0, 2);
            }
            else if (one_hz_str.ToUpper() == "Z")  //Z 开头声母, Z,ZH
            {
                if (tmpStr.Substring(0, 2).ToUpper() == "ZH")
                    one_hz_str = tmpStr.Substring(0, 2);
            }
