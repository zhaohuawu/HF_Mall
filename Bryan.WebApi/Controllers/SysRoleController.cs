...
namespace Bryan.WebApi.Controllers
{
    ...
    public class SysRoleController : BaseController
    {
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        [Permission("sys:menu:addmenu|sys:menu:editmenu")]
        [HttpPost]
        ...
        /// <returns></returns>
        [Permission("sys:menu:editmenuorders")]
        [HttpPost]
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        [Permission("sys:menu:addmenubtn")]
        [HttpPost]
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        [Permission("sys:menu:deletemenubtn")]
        [HttpDelete]
        ...
        /// <returns></returns>
        [Permission("sys:cache:redis|sys:menu:editmenu|sys:menu:deletemenubtn")]
        [HttpPost]
        public async Task<IActionResult> UpdateRolePerToRedis([FromForm]int isAdd, [FromForm]int menuId, [FromForm]int btnId)
        {
            var result = await Task.Run(() =>
            {
                var list = new List<PermissionToReloDto>();
                var dicList = new ConcurrentBag<Dictionary<int, List<PermissionToReloDto>>>();
                if (menuId > 0 && isAdd == 1)
                {
                    //给角色新增了菜单权限
                    ...
                    Parallel.ForEach(arr, role =>
                    {
                        ...
                        var dic = new Dictionary<int, List<PermissionToReloDto>> { { role, roleMenuList } };
                        dicList.Add(dic);
                    });
                }
                ...
                {
                    //停用菜单
                    var hDic = RedisHelper.HGetAll<List<PermissionToReloDto>>(RedisKeysEnum.RoleMenuHash.GetHFMallKey());
                    Parallel.ForEach(hDic, item =>
                    {
                        ...
                        if (roleRedis != null)
                        {
                            ...
                            var dic = new Dictionary<int, List<PermissionToReloDto>> { { Convert.ToInt32(item.Key), roleList } };
                            dicList.Add(dic);
                        }
                    });
                }
                ...
                else
                {
                    ...
                    Parallel.ForEach(roleList, item =>
                    {
                        ...
                        var dic = new Dictionary<int, List<PermissionToReloDto>> { { item, arr } };
                        dicList.Add(dic);
                    });
                }
